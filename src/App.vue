                ...
                <div class="item-list-wrap">
                    <div class="cart-item">
                        ...
                        <ul class="cart-item-list">
                            <li v-for="(i,index) in itemList">
                                <div class="cart-tab-1">
                                    <!-- 单选 -->
                                    <div class="cart-item-check">
                                        <a href="javascipt:;" class="item-check-btn" v-bind:class='{check:i.checked}' v-on:click='selectItem(i)'>
                                            <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                                        </a>
                                    </div>
                                    <!-- 商品图片 -->
                                    <div class="cart-item-pic">
                                        <img v-bind:src="i.productImage" v-bind:alt="i.productName">
                                    </div>
                                    <!-- 商品名称 -->
                                    <div class="cart-item-title">
                                        <div class="item-name" v-text='i.productName+index'></div>
                                    </div>
                                    <!-- 赠品 -->
                                    <div class="item-include">
                                        <dl>
                                            <dt>赠送:</dt>
                                            <dd v-for='j in i.parts' v-text='j.partsName'></dd>
                                        </dl>
                                    </div>
                                </div>
                                <!-- 单价 -->
                                <div class="cart-tab-2">
                                    <div class="item-price">{{i.productPrice | money}}</div>
                                </div>
                                <div class="cart-tab-3">
                                    <div class="item-quantity">
                                        <div class="select-self select-self-open">
                                            <div class="quentity">
                                                <a href="javascript:;" v-on:click='changeQuantity(i,-1)'>-</a>
                                                <input type="text"  v-model='i.productQuentity'>
                                                <a href="javascript:;" v-on:click='changeQuantity(i,1)'>+</a>
                                            </div>
                                        </div>
                                ...
                                <div class="cart-tab-4">
                                    <!-- 总金额 -->
                                    <div class="item-price-total">{{i.productQuentity*i.productPrice | money}}</div>
                                </div>
                                <!-- 删除功能 -->
                ...
                <div class="cart-foot-wrap">
                    <div class="cart-foot-l">
                        <div class="item-all-check">
                            <a href="javascipt:;">
                                <span class="item-check-btn" v-bind:class='{check:checkAll}' v-on:click='selectAll(true)'>
                                    <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                                </span>
                                <span>全选</span>
                            </a>
                        ...
                        <div class="item-all-del">
                            <a href="javascipt:;" class="item-del-btn" v-on:click='selectAll(false)'>
                                取消全选
                            </a>
                        </div>
                    </div>
...
export default {
    ...
    methods:{
        loadList:function(){
            console.log(this)
            this.$http.get('static/cart.json').then(function(back){
                this.itemList = back.body.result.productList;
                this.totalMoney = back.body.result.totalMoney;
            })
        },
        changeQuantity:function(i,num){
            if (num > 0) { 
                i.productQuentity++;
            }else{
                i.productQuentity--;
                if (i.productQuentity < 1) {
                    i.productQuentity = 1;
                }
            }
        },
        ...
        selectAll:function(type){
            ...
            this.itemList.forEach(function(val,index){
                if (typeof val.checked == 'undefined') {
                   _this.$set(val,'checked',_this.checkAll);
                }else{
                    val.checked = _this.checkAll;
                }
            })
        }
    }
